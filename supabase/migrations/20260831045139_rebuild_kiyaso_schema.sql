/*
# Rebuild Kiyaso Restaurant Schema (from scratch)

## Overview
User deleted all tables. This migration recreates the complete database schema,
storage buckets, RLS policies, realtime publication, and seed demo data.

## New Tables
1. categories - Menu categories with image
2. menu_items - Dishes with category FK, gallery array, ingredients array
3. branches - Restaurant locations with opening hours
...
12. settings - Singleton restaurant settings with logo + social links

## Storage
- Creates `kiyaso-images` storage bucket for all uploaded images
- Public read, anon write policies

## Realtime
- Enables realtime on all tables via publication

## Security
- RLS enabled on ALL tables, TO anon+authenticated (no-auth single-tenant app)
- Storage bucket public read + anon write
...
-- Menu Items (using category IDs from above)
